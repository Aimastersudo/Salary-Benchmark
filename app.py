...
# 2. DATABASE LOADER (The Core Logic from your original app.py)
@st.cache_data
def load_databases():
    try:
        ...
        # Audit Metadata for Transparency Lab
        ...
        # 200 HC Fix Logic (Original Correct Version)
...
if df is not None:
    ...
    if page == "📊 Executive Dashboard":
        ...
        st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>AI Insight:</b> {row['Designation']} is {abs(int(row['Variance %']))}% {'below' if row['Variance %'] < 0 else 'above'} market.</div></div>""", unsafe_allow_html=True)
    ...
    # 👥 PCI EMPLOYEES
        ...
        st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>AI Payroll Health Check:</b> {len(f_emp[f_emp['Gap %'] < -10])} employees are significantly underpaid. Tenure analysis identifies <b>{len(f_emp[(f_emp['Tenure_Y'] >= 3) & (f_emp['Gap %'] < -10)])}</b> loyal personnel at risk.</div></div>""", unsafe_allow_html=True)
    ...
    # 🎯 TRANSPARENCY LAB
        ...
        roles_list = ["-- Methodology Overview --"] + sorted(f_df['Designation'].unique().tolist())
        sel_role = st.selectbox("Select Audit Target:", roles_list)
        
        if sel_role == "-- Methodology Overview --":
            if not meth_db.empty:
                cols_m = st.columns(2)
                for idx, r_m in meth_db.iterrows():
                    with cols_m[idx % 2]: st.markdown(f"""<div class="meth-card"><span style="color:#38bdf8;font-weight:bold;">{r_m['Step']}. {r_m['Header']}</span><p style="font-size:14px;color:#94a3b8;">{r_m['Description']}</p></div><br>""", unsafe_allow_html=True)
        else:
            ...
            for i, c in enumerate(comp_cols):
                ...
                with chips[i]:
                    ...
                    else: st.markdown(f"""<div class="audit-card" style="opacity:0.5;"><small>{c}</small><br><b>N/A</b></div>""", unsafe_allow_html=True)

    # 📈 INCREMENT PLANNER
    elif page == "📈 Increment Planner":
        st.title("Increment Strategy Simulator")
        target = st.selectbox("Select Employee:", sorted(f_emp['Employee Name'].unique()))
