...
def generate_graphical_pdf(f_df, avg_v, worst_d, total_hc, crit_df, loyalty_count):
    ...
    pdf.set_font("Arial", '', 10); pdf.cell(190, 5, f"Snapshot Date: {datetime.now().strftime('%d %b %Y')}", 0, 1, 'C')
    ...
    summary = (f"Overall Parity: {avg_v}% | Total Staff: {total_hc} | Roles: {len(f_df)}\n\n"
               f"AI Strategic Advice: The current gap is {avg_v}%. The {worst_d} department is at critical risk. "
               f"High-priority alignment recommended for {loyalty_count} loyal personnel (5y+).")
    pdf.multi_cell(190, 8, summary, 1); pdf.ln(10)
    pdf.set_font("Arial", 'B', 14); pdf.set_fill_color(255, 230, 230)
    pdf.cell(190, 10, " 2. Priority Retention Gaps", 1, 1, 'L', True)
...
def load_databases():
    try:
        ...
        # 🚀 FIX: Do not fillna(0) here, keep NaN for filtering
        ...
        # 🚀 FIX: Fallback to pioneer salary if no market data
        ...
        emp_data = pd.merge(payroll_df, m_clean, on='Match
