...
def generate_graphical_pdf(f_df, avg_v, worst_d, total_hc, crit_df, loyalty_count):
    ...
    summary = (f"Market Disparity: {avg_v}% | Total Employees Analyzed: {total_hc} | Designations: {len(f_df)}\n\n"
               ...
               f"The {worst_d} department is identified as the highest talent retention risk. "
               f"Loyal workforce (5y+) includes {loyalty_count} employees who require immediate alignment review.")
...
if df is not None:
    ...
    with st.sidebar:
        ...
        if st.button("Generate Strategy Report"):
            ...
            st.download_button(label="📥 Download Strategy PDF", data=pdf_bytes, file_name="PCI_Salary_Report.pdf", mime="application/pdf")
    ...
    if page == "📊 Executive Dashboard":
        ...
        if sel_role:
            ...
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>Gemini HR Insight:</b> {row['Designation']} is {abs(row['Variance %'])}% {'below' if row['Variance %'] < 0 else 'above'} market avg.</div></div>""", unsafe_allow_html=True)
    ...
    # 📉 MARKET ANALYSIS
        ...
        if not f_df.empty:
            ...
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>AI Market Summary:</b> Pioneer is {abs(avg_var)}% behind market. Highest risk: {worst_d}.</div></div>""", unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1:
    ...
    # 👥 PCI EMPLOYEES
        ...
        if not f_emp.empty:
            ...
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>Gemini Payroll Health Check:</b> {len(f_emp[f_emp['Gap %'] < -10])} employees are significantly underpaid. Tenure vs Salary Analysis suggests attention needed for staff with 5+ years experience.</div></div>""", unsafe_allow_html=True)
