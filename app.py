...
def generate_graphical_pdf(f_df, avg_v, worst_d, total_hc, crit_df, loyalty_count):
    ...
    pdf.cell(190, 5, f"Analysis Snapshot: {datetime.now().strftime('%d %b %Y | %H:%M')}", 0, 1, 'C')
    ...
    summary = (f"Market Disparity: {avg_v}% | Total Employees Analyzed: {total_hc} | Designations: {len(f_df)}\n\n"
               f"AI Strategic Summary: Pioneer Cement shows a significant parity gap of {avg_v}% against competitors "
               f"like JK Cement and Union Cement. The {worst_d} department is identified as the highest talent retention "
               f"risk. Loyal workforce (5y+) includes {loyalty_count} employees who require immediate alignment review.")
    pdf.multi_cell(190, 8, summary, 1)
    
    # Section 2: Critical Gaps Table
    pdf.ln(10)
    pdf.set_font("Arial", 'B', 14)
    pdf.set_fill_color(255, 230, 230)
    pdf.cell(190, 10, " 2. Critical Retention Risk (Top Gaps)", 1, 1, 'L', True)
    pdf.set_font("Arial", 'B', 10)
    pdf.cell(90, 8, "Designation", 1); pdf.cell(50, 8, "Department", 1); pdf.cell(30, 8, "Market Gap", 1); pdf.cell(20, 8, "HC", 1, 1)
    
    pdf.set_font("Arial", '', 9)
    for _, row in crit_df.head(12).iterrows():
        pdf.cell(90, 7, str(row['Designation']), 1)
        pdf.cell(50, 7, str(row['Department']), 1)
        pdf.cell(30, 7, f"{row['Variance %']}%", 1)
        pdf.cell(20, 7, str(row['Live_HC']), 1, 1)
        
    pdf.ln(15)
    pdf.set_font("Arial", 'I', 8)
    pdf.cell(190, 5, "CONFIDENTIAL REPORT - GENERATED BY PIONEER AI SYSTEM", 0, 1, 'C')
...
def load_databases():
    try:
        ...
        # 200 HC Residual Allocation Logic
...
if df is not None:
    ...
    with st.sidebar:
        ...
        if st.button("Generate Strategy Report"):
            ...
            avg_v = int(f_pdf['Variance %'].mean())
            worst_d = f_pdf.groupby('Department')['Variance %'].mean().idxmin()
            ...
            st.download_button(label="📥 Download Graphical PDF", data=pdf_bytes, file_name="PCI_Salary_Analysis.pdf", mime="application/pdf")
    ...
    if page == "📊 Executive Dashboard":
        ...
        avg_v = f"{int(f_df['Variance %'].mean())}%"
    ...
    # 📉 MARKET ANALYSIS
        ...
        if not f_df.empty:
            ...
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>AI Market Summary:</b> Pioneer is {abs(avg_var)}% behind market. Highest risk: {worst_d}. Bubble size indicates headcount focus.</div></div>""", unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1:
    ...
    # 👥 PCI EMPLOYEES
        ...
        if not f_emp.empty:
            ...
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>Gemini Payroll Health Check:</b> {len(f_emp[f_emp['Gap %'] < -10])} employees are significantly underpaid. Tenure vs Salary Analysis suggests attention needed for long-term staff with high gaps.</div></div>""", unsafe_allow_html=True)

            # Highlight Cards
            h1, h2 = st.columns(2)
            with h1: st.error(f"⚠️ Top Underpaid: {f_emp.sort_values('Gap %').iloc[0]['Employee Name']} ({f_emp.sort_values('Gap %').iloc[0]['Gap %']}%)")
            ...
            if sel_name:
                ...
                with ca:
                    st.markdown(f"""<div class="profile-card"><h3>{ed['Employee Name']}</h3><p>ID: {ed['Employee ID']} | Tenure: {ed['Tenure_Text']}</p><hr><p>Salary: {int(ed['Salary'])} AED | <span class="{'highlight-red' if ed['Gap %'] < 0 else 'highlight-green'}">Gap: {ed['Gap %']}%</span></p></div>""", unsafe_allow_html=True)
                with cb:
    ...
    # 4. INCREMENT PLANNER
        ...
        if target:
            ...
            with col2:
                st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>AI Budget Strategy:</b> Increment aligns {data['Employee Name']} closer to market. Monthly impact: {new_s - int(data['Salary']):,} AED.</div></div>""", unsafe_allow_html=True)
                fig = go.Figure(go.Indicator(mode="gauge+number", value=new_s, title={'text': "Market Alignment (Gauge)"}, gauge={'axis': {'range': [None, data['Market_Avg']*1.5]}, 'bar': {'color': "#3b82f6"}, 'steps': [{'range': [0, data['Market_Avg']*0.9], 'color': "red"}, {'range': [data['Market_Avg']*0.9, data['Market_Avg']*1.1], 'color': "green"}]}))
