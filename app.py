...
# 2. ADVANCED PDF GENERATOR (Visual Report)
def generate_visual_pdf(f_df, avg_v, worst_d, total_hc, crit_df, loyalty_count):
    pdf = FPDF()
    pdf.add_page()
    ...
    pdf.set_font("Arial", '', 10); pdf.cell(190, 5, f"Analysis Snapshot: {datetime.now().strftime('%d %b %Y')}", 0, 1, 'C')
    pdf.ln(25); pdf.set_text_color(0, 0, 0); pdf.set_font("Arial", 'B', 14); pdf.set_fill_color(230, 235, 255)
    pdf.cell(190, 10, " 1. Executive Summary & AI Insight", 1, 1, 'L', True)
    pdf.set_font("Arial", '', 11)
    summary = (f"Market Disparity: {avg_v}% | Total Employees: {total_hc} | Designations: {len(f_df)}\n\n"
               f"AI Strategic Advice: Pioneer Cement shows a parity gap of {avg_v}% against key competitors. "
               f"The {worst_d} department is the highest retention risk. Workforce includes {loyalty_count} loyal "
               f"personnel (5y+) requiring immediate market alignment review.")
    pdf.multi_cell(190, 8, summary, 1); pdf.ln(10)
    pdf.set_font("Arial", 'B', 14); pdf.set_fill_color(255, 230, 230)
    pdf.cell(190, 10, " 2. Critical High-Risk Roles", 1, 1, 'L', True)
    pdf.set_font("Arial", 'B', 10); pdf.cell(90, 8, "Designation", 1); pdf.cell(50, 8, "Dept", 1); pdf.cell(30, 8, "Gap %", 1); pdf.cell(20, 8, "HC", 1, 1)
...
# 3. TRIPLE DATABASE LOADER (Safe Rounding & 200 HC Engine)
@st.cache_data
def load_databases():
    try:
        ...
        market_df['Market_Avg'] = m_calc[comp_cols].mean(axis=1).round(0).fillna(0).astype(int)
        ...
        final_df['Live_HC'] = final_df['HC_Dept'] = final_df['HC_D'].fillna(0).astype(int)
...
if df is not None:
    ...
    with st.sidebar:
        ...
        if st.button("Generate Strategy Report"):
            ...
            crit_pdf = f_pdf[f_pdf['Variance %'] <= -20].sort_values('Variance %')
            loy_count = len(emp_df[emp_df['Tenure_Y'] >= 5])
            pdf_bytes = generate_visual_pdf(f_pdf, avg_v, worst_d, int(f_pdf['Live_HC'].sum()), crit_pdf, loy_count)
            st.download_button(label="📥 Download Graphical PDF", data=pdf_bytes, file_name="PCI_Salary_Report.pdf", mime="application/pdf")
    ...
    if page == "📊 Executive Dashboard":
        ...
        sel_role = st.selectbox("Select Role for Analysis:", f_df['Designation'].unique())
        if sel_role:
            row = f_df[f_df['Designation'] == sel_role].iloc[0]
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>Gemini HR Analysis:</b> {row['Designation']} in the {row['Department']} department is {abs(int(row['Variance %']))}% {'below' if row['Variance %'] < 0 else 'above'} market benchmark. Retention risk: {'High' if row['Variance %'] < -20 else 'Moderate'}.</div></div>""", unsafe_allow_html=True)
    ...
    # 2. MARKET ANALYSIS
        ...
        if not f_df.empty:
            mean_v = f_df['Variance %'].mean(); avg_var = int(mean_v) if pd.notna(mean_v) else 0
            worst_d = f_df.groupby('Department')['Variance %'].mean().idxmin()
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>AI Market Intelligence:</b> Pioneer Cement is {abs(avg_var)}% behind benchmarks. {worst_d} is the most vulnerable department. Bubble size indicates workforce density.</div></div>""", unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1:
            ...
            st.subheader("⚠️ Critical Priority Adjustment Table")
    ...
    # 3. PCI EMPLOYEES (Interface with Health Check)
        ...
        if not f_emp.empty:
            ...
            # 🚀 AI PAYROLL HEALTH CHECK
            st.markdown(f"""
            <div class="salary-card">
                <div class="ai-insight-box">
                    <b>AI Payroll Health Check:</b> {len(f_emp[f_emp['Gap %'] < -10])} employees are significantly underpaid. 
                    Tenure vs Salary Analysis indicates <b>{len(f_emp[(f_emp['Tenure_Y'] >= 3) & (f_emp['Gap %'] < -10)])}</b> loyal employees 
                    (3y+) are at critical retention risk due to high market gaps.
                </div>
            </div>
            """, unsafe_allow_html=True)
            ...
            if sel_name:
                ...
                with ca:
                    st.markdown(f"""<div class="profile-card"><h3>{ed['Employee Name']}</h3><p>ID: {ed['Employee ID']} | Tenure: {ed['Tenure_Text']}</p><p>Joined: {ed['Date of Joining']}</p><hr><p>Salary: {int(ed['Salary']):,} AED | <span class="{'highlight-red' if ed['Gap %'] < 0 else 'highlight-green'}">{'Underpaid' if ed['Gap %'] < 0 else 'Above Market'} ({int(ed['Gap %'])}%)</span></p></div>""", unsafe_allow_html=True)
                with cb:
    ...
    # 4. INCREMENT PLANNER
        ...
        if target:
            ...
            with col1:
                ...
                st.metric("Market Position After", f"{gap_af}% Gap")
            with col2:
                st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>AI Budget Strategy:</b> Incrementing {data['Employee Name']} aligns them closer to market. Status after: <b>{'Still Underpaid' if gap_af < -5 else 'Market Aligned'}</b>. Monthly impact: {new_s - int(data['Salary']):,} AED.</div></div>""", unsafe_allow_html=True)
