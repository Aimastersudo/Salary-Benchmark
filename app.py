...
if df is not None:
    ...
    if page == "📊 Executive Dashboard":
        ...
        if sel_role:
            ...
            for i, c in enumerate(comp_cols):
                ...
                with cols[i]:
                    if pd.notna(val):
                        st.markdown(f"""<div class="market-box"><small>{c}</small><br><b style="color:#38bdf8;">{int(val):,}</b></div>""", unsafe_allow_html=True)
                    else:
    ...
    # 👥 3. PCI EMPLOYEES
        ...
        if not f_emp.empty:
            ...
            if sel_name:
                ...
                with ca: st.markdown(f"""<div class="profile-card"><h3>{ed['Employee Name']}</h3><p>{ed['Designation']}</p><hr><p>Salary: {int(ed['Salary']):,} AED | Gap: {int(ed['Gap %'])}%</p></div>""", unsafe_allow_html=True)
                with cb:
                    ...
                    for i, cn in enumerate(comp_cols):
                        ...
                        with cc[i]:
                            if pd.notna(cv):
                                st.markdown(f"""<div class="market-box"><small>{cn}</small><br><b style="color:#38bdf8;">{int(cv):,}</b></div>""", unsafe_allow_html=True)
                            else:
                                st.markdown(f"""<div class="market-box"><small>{cn}</small><br><span style="color:#4b5563;">N/A</span></div>""", unsafe_allow_html=True)
            st.divider(); st.dataframe(f_emp[['Employee ID', 'Employee Name', 'Designation', 'Department', 'Salary', 'Market_Avg', 'Gap %']], use_container_width=True, hide_index=True)
    ...
    # 🎯 4. TRANSPARENCY LAB
        ...
        if sel_role == "-- Select Designation --":
            st.markdown("""<div class="ai-insight-box"><b>Strategic Methodology:</b> documentation of the computational logic used.</div>""", unsafe_allow_html=True)
            if not meth_db.empty:
                cols_meth = st.columns(2)
                for i, row_meth in meth_db.iterrows():
                    with cols_meth[i % 2]:
                        st.markdown(f"""<div class="meth-card"><span class="meth-header">{row_meth['Step']}. {row_meth['Header']}</span><p style="font-size: 14px; color: #94a3b8;">{row_meth['Description']}</p></div><br>""", unsafe_allow_html=True)
        else:
