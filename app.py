...
# 2. ADVANCED PDF GENERATOR (Graphical Report)
def generate_graphical_pdf(f_df, avg_v, worst_d, total_hc, crit_df, loyalty_count):
    ...
    summary = (f"Market Disparity: {avg_v}% | Total Employees: {total_hc} | Designations: {len(f_df)}\n\n"
               f"AI Summary: Pioneer Cement has a {avg_v}% parity gap. {worst_d} is the highest risk dept. "
               f"There are {loyalty_count} loyal employees (5y+) needing alignment.")
    pdf.multi_cell(190, 8, summary, 1); pdf.ln(10)
    pdf.set_font("Arial", 'B', 14); pdf.set_fill_color(255, 230, 230)
    pdf.cell(190, 10, " 2. Critical Retention Risk Roles", 1, 1, 'L', True)
    pdf.set_font("Arial", 'B', 10); pdf.cell(90, 8, "Designation", 1); pdf.cell(50, 8, "Dept", 1); pdf.cell(30, 8, "Gap %", 1); pdf.cell(20, 8, "HC", 1, 1)
    pdf.set_font("Arial", '', 9)
    for _, row in crit_df.head(12).iterrows():
...
if df is not None:
    with st.sidebar:
        ...
        if st.button("Generate Strategy Report"):
            ...
            pdf_bytes = generate_graphical_pdf(f_pdf, avg_v, worst_d, int(f_pdf['Live_HC'].sum()), crit_df, loyalty_count)
    ...
    if page == "📊 Executive Dashboard":
        ...
        if sel_role:
            ...
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>Gemini HR Insight:</b> {row['Designation']} is {abs(int(row['Variance %']))}% {'below' if row['Variance %'] < 0 else 'above'} market avg.</div></div>""", unsafe_allow_html=True)
        ...
        if not f_emp.empty:
            ...
            st.markdown(f"""<div class="salary-card"><div class="ai-insight-box"><b>Gemini Health Check:</b> {len(f_emp[f_emp['Gap %'] < -10])} employees are significantly underpaid. Tenure vs Salary Analysis identifies long-term risk.</div></div>""", unsafe_allow_html=True)
            sel_name = st.selectbox("Spotlight Search:", sorted(f_emp['Employee Name'].unique()))
            if sel_name:
